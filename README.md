...
> **진행 상태:** 초기 세팅에 이어 회원가입/로그인, 첫 사용자 기능(주식 용어
> 사전)과 로그인 후 메인 대시보드(랜덤 용어 카드)까지 구현했습니다.
> 회원가입부터 로그인, 대시보드까지 프론트엔드 화면만으로 전부 이어지며
> (더 이상 Swagger가 없어도 됩니다), 이후 실제 기능 화면을 하나씩 추가해
> 나갈 예정입니다.
>
> **구현된 기능:** 회원가입, 로그인(세션 + Redis), 주식 용어 사전(개인별
> CRUD), 메인 대시보드(랜덤 카드 복습) — 자세한 내용은
