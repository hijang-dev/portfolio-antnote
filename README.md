...
> 사전)까지 구현했습니다. 백엔드/프론트엔드 뼈대가 서로 연동되어 동작하며
> (DB 연결, API, 클라이언트 상태 관리, 서버 데이터 패칭까지 확인 완료),
> 이후 실제 기능 화면을 하나씩 추가해 나갈 예정입니다.
>
> **구현된 기능:** 회원가입, 로그인(세션 + Redis), 주식 용어 사전(개인별
> CRUD) — 자세한 내용은 [`docs/FEATURES.md`](./docs/FEATURES.md) 참고
