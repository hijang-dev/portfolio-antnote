...
│   Next.js 클라이언트 │ ◀── 세션 쿠키(httpOnly) ── │  NestJS API 서버  │ ◀───────────────── │              │
└───────────────────┘                            └────────┬─────────┘                    └──────────────┘
      Zustand                                              │ 세션 조회/저장
   (클라이언트/UI 상태)                                       ▼
   TanStack Query                                    ┌──────────────┐
...
- 웹 클라이언트는 PostgreSQL/Redis와 절대 직접 통신하지 않습니다 — 모든
  데이터 접근은 NestJS API를 거칩니다.
...
- 인증은 JWT가 아니라 **세션 + Redis**로 구현했습니다. 로그인 상태를
  서버가 직접 들고 있으므로 로그아웃이 즉시 반영되고, 클라이언트에는
  서명된 세션 ID만 `httpOnly` 쿠키로 내려갑니다 (자세한 트레이드오프는
  [기능 구현 가이드 — 로그인](./FEATURES.md) 참고).
